...
12. **Top 14 — full real format, closed league (CEO, July 2026).** League three ships with the real structure: 14 clubs, 26 rounds (double round-robin), French bonus rules (offensive bonus for out-scoring the opponent by 3+ tries; losing bonus within 5), **top-6 playoffs with the barrage round** (3v6, 4v5 at the higher seed; semi-finals vs seeds 1–2; final at Stade de France), which extends the shared calendar dynamically (~30 weeks in a Top 14 world — season length is now derived per-competition, not fixed at 22). **No relegation:** ProD2 does not exist in the game world, so the Top 14 is a closed league like the Prem and URC; revisit only if a second division ever ships. Salary cap is the uniform real cap (≈ €10.7m ≈ £9.0m), making the Top 14 the richest league and the "galactico" destination per §4. Save format bumps (v3 saves discarded behind the standard fresh-career prompt, pre-release only).
